    ...
    $(e).hover(
      function () {
        ...
        e &&
          ...
          $("<span/>", { text: e, class: "tooltiptext" }).appendTo(this));
      },
      function () {
...
async function addItinerary() {
  ...
  itineraryContainer.insertBefore(a, itineraryContainer.firstChild),
    e.unshift(t),
    await saveDataInStorage(SAVED_ITINERARY, e),
    (await loadDataFromStorage(RIGHT_ITINERARY)) || updateRightSection(a);
}
...
function createSwitchControl(e, t) {
  t = jQuery.parseHTML(`<div class="noEffect">
      <label class="switch">
        <input type="checkbox" id="mySwitch" data-metadata-key="${t.key}" ${
    e ? "checked" : ""
  }>
  ...
  return (
    r.addEventListener("click", async (e) => {
      ...
      if (e.target.checked)
        for (let e = 0; e < a.length; e++) a[e] != r && (a[e].checked = !1);
      let i = {};
      ...
      await saveDataInStorage(SAVED_ITINERARY, n),
        await saveDataInStorage(ACTIVE_SELECTION_KEY, i);
    }),
    t
  );
}
function createIteneraryHtml(e) {
  ...
  t.appendChild(a),
    a.setAttribute("data-metadata-key", e.key),
    updateItemTitle(t, e),
    a.classList.add("expandWidth"),
    t.classList.add("apart"),
    t.setAttribute("data-metadata-key", e.key);
...
function itineraryToItem(t) {
  ...
  for (let e = 0; e < a.length; e++)
    if (a[e].getAttribute("data-metadata-key") === t) return a[e];
  return null;
}
...
async function updateRightSection(n) {
  if (n) {
    ...
    (e.value = (o.date && o.date.split("/").reverse().join("-")) || ""),
      (e.oninput = function (e) {
        (o.date = e.target.value.split("-").reverse().join("/")),
          updateItemTitle(n, o),
          saveItinerary(o);
      }),
    ...
    var e = document.getElementById(a[0]),
      e =
        ((e.onchange = function (e) {
          (o.paymentMode = e.target.value),
            d("paymentMode", "paymentType"),
            r("paymentType", o),
            d("paymentType", "paymentGateway"),
            r("paymentGateway", o),
            saveItinerary(o);
        }),
    ...
    function r(e, t) {
      ...
      Array.from(a.options)[a.selectedIndex].hasAttribute("hidden") &&
        (a.value = Array.from(a.options).filter(
    ...
    function d(e, a) {
      if (a && e) {
        ...
        if (a && e) {
          let t = Array.from(e.options)[e.selectedIndex].getAttribute(
            "data-option"
          );
          Array.from(a.options).forEach((e) => {
            e.getAttribute("data-parent-option") === t
    ...
    editorContainer.setAttribute("hidden", !0),
      deleteButton.setAttribute("hidden", !0),
      messageContainer.removeAttribute("hidden"),
      addButton.setAttribute("hidden", !0);
}
createKeyDocButton.addEventListener("click", async function () {
  window
    .open("https://ai.google.dev/gemini-api/docs/api-key", "_blank")
    .focus();
}),
  aboutMeButton.addEventListener("click", async function () {
    window.open("https://github.com/rksharma007", "_blank");
  }),
  ...
  deleteButton.addEventListener("click", async function () {
    ...
    for (let e = 0; e < n.length; e++)
      n[e].getAttribute("data-metadata-key") === t && n[e].remove();
    a.active && (await saveDataInStorage(ACTIVE_SELECTION_KEY, {})),
      await saveDataInStorage(RIGHT_ITINERARY, ""),
      updateRightSection(itineraryContainer.children[0]);
  }),
  addPassengerButton.addEventListener("click", async function () {
    try {
      let t = await loadDataFromStorage(RIGHT_ITINERARY);
      var e = (await loadDataFromStorage(SAVED_ITINERARY)).find(
        (e) => e.key === t
      );
      e.passengers.push({
        name: "",
        ...
        await saveItinerary(e),
        updateRightSection(itineraryToItem(t));
    } catch (e) {
      console.error(e);
    }
  }),
